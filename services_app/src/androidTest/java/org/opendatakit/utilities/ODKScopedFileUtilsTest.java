...
//    }
    ...
    @Test
    public void getAsFile_withValidAppNameAndUriFragment_returnsFile() {
        System.out.println(getMediaStoreUri(context, ODK_FOLDER_NAME));
        System.out.println("File saved to scoped storage");
//        Uri folderUri = Uri.parse(ODK_FOLDER_PATH).buildUpon()
//                .appendEncodedPath(TEST_APP + PATH_SEPARATOR + URI_FRAGMENT).build();
//        DocumentFile expectedFile = DocumentFile.fromTreeUri(context, folderUri);
//        assertEquals(expectedFile, ODKFileUtils.getAsFile(TEST_APP, URI_FRAGMENT));
//        expectedFile = new File(ODK_FOLDER, TEST_APP + PATH_SEPARATOR + URI_FRAGMENT + TEST_FILE);
        assertEquals(getMediaStoreUri(context, ODK_FOLDER_NAME), Uri.fromFile(ODKFileUtils.getAsFile(TEST_APP, URI_FRAGMENT + TEST_FILE)));
    }
